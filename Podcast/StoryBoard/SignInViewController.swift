...
class SignInViewController: UIViewController, UITextFieldDelegate{
    ...
    @objc func handleSignIn() {
        guard let email = emailField.text,email != "" ,let pass = passwordField.text , pass != "" else {

            let logingAlertController  = UIAlertController(title: "Loging Error", message: "Fields must not be blank ", preferredStyle: .alert)
            let  logingAction = UIAlertAction(title: "Ok", style: .cancel, handler: nil)
            logingAlertController.addAction(logingAction)
            present(logingAlertController,animated: true,completion: nil)
            return
        }
